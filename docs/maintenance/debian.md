...
1. **Prepare the Directory**:
   ```bash
   mkdir -p build/rustyping_2.4.1_amd64/DEBIAN
   mkdir -p build/rustyping_2.4.1_amd64/usr/local/bin
   ```

2. **Copy Binary**:
   Assuming you have the linux binary in `target/release/rustyping`:
   ```bash
   cp target/release/rustyping build/rustyping_2.4.1_amd64/usr/local/bin/
   chmod 755 build/rustyping_2.4.1_amd64/usr/local/bin/rustyping
   ```

3. **Create Control File**:
   Copy the `packaging/debian/control` file and update the version number if necessary.
   ```bash
   cp packaging/debian/control build/rustyping_2.4.1_amd64/DEBIAN/
   ```

4. **Build the .deb**:
   ```bash
   dpkg-deb --build build/rustyping_2.4.1_amd64
   ```
   This will output `build/rustyping_2.4.1_amd64.deb`.
...
```bash
wget https://github.com/pdzjtechnagy/RustyPing/releases/download/v2.4.1/rustyping_2.4.1_amd64.deb
sudo dpkg -i rustyping_2.4.1_amd64.deb
```
